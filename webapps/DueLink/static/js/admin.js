...
function send_form() {
    ...
    console.log(dl_datetime);
    $.post("publish_deadline", {deadline_datetime: dl_datetime, name: dl_name, course: dl_course})
        .done(function() {
            alert("Success: new evnet added");

        })
        .fail(function(){
            alert("Fail to add new event, deadline should not be duplicated or earlier than current time");
        });
}
